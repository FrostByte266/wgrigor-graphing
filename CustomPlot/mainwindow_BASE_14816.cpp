...
void MainWindow::addPoints(double xval){
    double yval;
...
void MainWindow::setRange(double xyrange){
...
void MainWindow::on_submitRange_clicked()
{
    ...
    if(ui->varValXincpt->QDoubleSpinBox::value() !=0 || ui->varValYincpt->QDoubleSpinBox::value() !=0){
        clearGraph();
        placeXint();
        placeYint();
        setSlope();
